...
function FindingDriver() {
  ...
  // Handle search timeout (5 minutes with no driver)
  const handleSearchTimeout = async () => {
    try {
      // Cancel the pending ride
      await api.put(`/rides/cancel-pending/${rideId}`);
      setIsCancelled(true);
    } catch (error) {
        ...
        >
          ...
          <div className="p-6 border-b border-gray-200">
            {driverFound ? (
              ...
              <div className="text-center">
                <h2 className="text-lg font-medium text-gray-900">Finding you a driver</h2>
                <p className="mt-1 text-sm text-gray-500">
              ...
              <div className="flex">
                ...
                <div>
                  ...
                  <p className="font-medium">
                    {rideDetails ? (() => {
                      console.log('Full ride details:', rideDetails);
                      console.log('Calculating seats with leftSeats:', rideDetails.leftSeats);
                      
                      const totalSeats = 4; // Always 4 seats total in the system
                      
                      // Calculate booked seats - try availableSeats first, then calculate from leftSeats
                      let bookedSeats;
                      if (rideDetails.availableSeats !== undefined) {
                        ...
                        bookedSeats = 1; // Default to 1 if we can't determine
                      }
                      
                      console.log('Using bookedSeats:', bookedSeats);
          ...
          {driverFound && !showDriverModal && (
            <div className="px-6 py-4 bg-gray-50 border-t border-gray-200">
              <a
                href="/my-trips"
                className="w-full py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-green-600 hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 flex items-center justify-center"
              >
