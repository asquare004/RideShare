      ...
      try {
        ...
        formattedRides.forEach(trip => {
          ...
          if (trip.status === 'completed' || trip.status === 'cancelled') {
            // For completed or cancelled rides, add them to completed list
            console.log(`Adding ${trip.status} ride to completed list:`, trip.id);
            completed.push(trip);
          } else if (trip.status === 'scheduled' || trip.status === 'pending') {
            // For scheduled or pending rides, check the date
            if (rideDateTime > currentDate) {
              // Upcoming ride
              console.log(`Adding ${trip.status} ride to upcoming list:`, trip.id);
              upcoming.push(trip);
            } else {
              // Ride date has passed but status wasn't updated
              console.log(`Adding past ${trip.status} ride to completed list:`, trip.id);
              completed.push({...trip, status: 'completed'});
            }
          } else {
            // For any unknown status, fall back to date-based categorization
            if (rideDateTime > currentDate) {
              upcoming.push(trip);
            } else {
              completed.push({...trip, status: 'completed'});
